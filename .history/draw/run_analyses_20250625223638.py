...
def load_your_models():
    ...
    model_configs = {
        'LIF': {
            'checkpoint': 'D:\FPTU-sourse\Term4\ResFes\Spike-Driven-Transformer_newSPS\Trained\Origin\Ori_b64\model_best.pth.tar',  # Update path này
            'config': {
                'img_size_h': 32,
                'img_size_w': 32,
                'patch_size': 4,
                'in_channels': 3,
                'num_classes': 10,
                'embed_dims': 256,  # Hoặc 512, check config của bạn
                'num_heads': 8,
                'mlp_ratios': 4,
                'depths': 2,  # Hoặc số layers bạn dùng
                'T': 4,  # time steps
                'spike_mode': 'lif'
            }
        },
        'IF_Hard': {
            'checkpoint': 'D:\FPTU-sourse\Term4\ResFes\Spike-Driven-Transformer_newSPS\Trained\IF_hard\b64_Uth1_2\model_best.pth.tar',  # Update path này
            'config': {
                'img_size_h': 32,
                'img_size_w': 32,
                'patch_size': 4,
        ...
        'IF_Soft': {
            'checkpoint': 'D:\FPTU-sourse\Term4\ResFes\Spike-Driven-Transformer_newSPS\Trained\IF_soft\b64_Uth1_2\model_best.pth.tar',  # Update path này
            'config': {
                'img_size_h': 32,
                'img_size_w': 32,
                'patch_size': 4,
    ...
    for model_name, info in model_configs.items():
        ...
        try:
            ...
            # Debug: Print checkpoint keys để hiểu structure
...
def inspect_checkpoint(checkpoint_path):
    """Helper function để inspect checkpoint structure"""
    try:
        checkpoint = torch.load(checkpoint_path, map_location='cpu', weights_only=False)
...
def setup_data_loader():
    ...
    # CIFAR-10 transforms (theo config trong yml files của bạn)
    ...
    test_loader = DataLoader(
        ...
        batch_size=32,
        shuffle=False,
        num_workers=2,  # Giảm xuống để tránh memory issues
...
def main():
    ...
    # Update these paths với actual checkpoint paths của bạn
    checkpoint_paths = [
