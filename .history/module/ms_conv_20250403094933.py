...
from module import MultiStepIFNodeSoftReset
...
class MS_MLP_Conv(nn.Module):
    def __init__(
        ...
        if spike_mode == "lif":
            ...
            self.fc1_lif = MultiStepIFNode(detach_reset=True, backend="cupy")
            self.fc2_lif = MultiStepIFNode(detach_reset=True, backend="cupy")
        elif spike_mode == "if_soft":
            self.fc1_lif = MultiStepIFNodeSoftReset(detach_reset=True)
            self.fc2_lif = MultiStepIFNodeSoftReset(detach_reset=True)
...
class MS_SSA_Conv(nn.Module):
    def __init__(
        ...
        if spike_mode == "if":
            self.shortcut_lif = MultiStepIFNode(detach_reset=True, backend="cupy")
            self.q_lif = MultiStepIFNode(detach_reset=True, backend="cupy")
            self.k_lif = MultiStepIFNode(detach_reset=True, backend="cupy")
            self.v_lif = MultiStepIFNode(detach_reset=True, backend="cupy")
            self.attn_lif = MultiStepIFNode(
                v_threshold=0.5, detach_reset=True, backend="cupy"
            )
            self.talking_heads_lif = MultiStepIFNode(
                v_threshold=0.5, detach_reset=True, backend="cupy"
            )
        if spike_mode == "if_soft":
            self.shortcut_lif = MultiStepIFNodeSoftReset(detach_reset=True)
            self.q_lif = MultiStepIFNodeSoftReset(detach_reset=True)
            self.k_lif = MultiStepIFNodeSoftReset(detach_reset=True)
            self.v_lif = MultiStepIFNodeSoftReset(detach_reset=True)
            self.attn_lif = MultiStepIFNodeSoftReset(
                v_threshold=0.5, detach_reset=True
            )
            self.talking_heads_lif = MultiStepIFNodeSoftReset(
                v_threshold=0.5, detach_reset=True
            )
