...
class MS_MLP_Conv(nn.Module):
    def __init__(
        ...
        elif spike_mode == "if" and v_reset != 0:
            self.fc1_lif = MultiStepIFNode(v_reset=v_reset, detach_reset=True, backend="cupy")
...
class MS_SSA_Conv(nn.Module):
    def __init__(
        ...
        if spike_mode == "if" and v_reset != 0:
            self.shortcut_lif = MultiStepIFNode(v_reset=v_reset, detach_reset=True, backend="cupy")
            self.q_lif = MultiStepIFNode(v_reset=v_reset, detach_reset=True, backend="cupy")
            self.k_lif = MultiStepIFNode(v_reset=v_reset, detach_reset=True, backend="cupy")
            self.v_lif = MultiStepIFNode(v_reset=v_reset, detach_reset=True, backend="cupy")
