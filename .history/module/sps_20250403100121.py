...
class MultiStepIFNodeSoftReset(nn.Module):
    ...
    def forward(self, x_seq):
        if len(x_seq.shape) != 5:
         raise ValueError(f"Expected input shape (T, B, C, H, W), but got {x_seq.shape}")
        # x_seq: (T, B, C, H, W)
        ...
        for t in range(T):
            ...
            v = v - spike * self.v_threshold  # Reset mềm: V = V - ngưỡng phát xung
...
class MS_SPS(nn.Module):
    def __init__(
        ...
        if spike_mode == "if":
          self.proj_lif = MultiStepIFNode(detach_reset=True, backend="cupy")
        elif spike_mode == "if_soft":
            self.proj_lif = MultiStepIFNodeSoftReset(detach_reset=True)
        ...
        if spike_mode == "if":
          self.proj_lif1 = MultiStepIFNode(detach_reset=True, backend="cupy")
        elif spike_mode == "if_soft":
            self.proj_lif1 = MultiStepIFNodeSoftReset(detach_reset=True)
        ...
        if spike_mode == "if":
          self.proj_lif2 = MultiStepIFNode(detach_reset=True, backend="cupy")
        elif spike_mode == "if_soft":
            self.proj_lif2 = MultiStepIFNodeSoftReset(detach_reset=True)
        ...
        if spike_mode == "if":
          self.proj_lif3 = MultiStepIFNode(detach_reset=True, backend="cupy")
        elif spike_mode == "if_soft":
            self.proj_lif3 = MultiStepIFNodeSoftReset(detach_reset=True)
        ...
        if spike_mode == "if":
          self.rpe_lif = MultiStepIFNode(detach_reset=True, backend="cupy")
        elif spike_mode == "if_soft":
            self.rpe_lif = MultiStepIFNodeSoftReset(detach_reset=True)
