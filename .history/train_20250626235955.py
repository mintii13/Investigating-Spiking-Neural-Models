...
from module.encoder import SimplePoisson, PoissonEncoder
...
parser.add_argument(
    "--resume",
    default=r"D:\FPTU-sourse\Term4\ResFes\Spike-Driven-Transformer_newSPS\output\train\20250626-224025-sdt-data-cifar10-t-4-spike-if_soft-rpe-sinusoidal\last.pth.tar",
    type=str,
    metavar="PATH",
...
parser.add_argument(
    ...
    default="none",
    type=str,
    choices=["simple_poisson", "poisson", "none"],
...
def train_one_epoch(
    ...
    # # Khởi tạo encoder nếu cần
    # if args.encoder_type == "simple_poisson":
    #     encoder = SimplePoisson(time_interval=args.time_steps)
    # elif args.encoder_type == "poisson":
    #     encoder = PoissonEncoder(time=args.time_steps, dt=1.0)
    # else:
    #     encoder = None
    for batch_idx, (input, target) in enumerate(loader):
        ...
        # if encoder != None:
        #     # In ra dữ liệu trước khi qua encoder (nếu chưa mã hóa)
        #     input = torch.sigmoid(input)  # Giới hạn giá trị trong khoảng [0, 1]
        #     if not args.encoder_type == "none" and batch_idx == 0:  # Thêm điều kiện batch_idx < 5
        #         print("\n--- Before Encoding ---")
        #         print("Input image shape:", input.shape)
        #         print("First few values of input image (channel 0, first 10x10 pixels):")
        #         print(input[0, 0, :8, :8])  # Channel đầu tiên, 10x10 pixel đầu

        #     # Áp dụng mã hóa nếu cần
        #     if encoder:
        #         input = encoder(input)

        #     # In ra dữ liệu sau khi qua encoder
        #     if encoder and batch_idx == 0:  # Thêm điều kiện batch_idx < 5
        #         print("\n--- After Encoding ---")
        #         print("Encoded image shape:", input.shape)

        #         # In ra giá trị đầu tiên của từng timestep (channel đầu tiên, 10x10 pixel đầu)
        #         print("\n--- First few values at each timestep (channel 0, first 10x10 pixels) ---")
        #         for t in range(input.shape[1]):  # Lặp qua từng timestep (dim=1)
        #             print(f"\nTimestep {t}:")
        #             print(input[0, t, 0, :8, :8])  # Batch 0, channel 0, 10x10 pixel đầu

        #         # Tính và in ra số lượng spike tại mỗi điểm ảnh (channel đầu tiên, 10x10 pixel đầu)
        #         spike_counts = torch.sum(input[:, :, 0, :, :], dim=1)  # Tổng số spike qua tất cả các timestep
        #         print("\n--- Spike Counts (channel 0, first 10x10 pixels) ---")
        #         print("Spike counts shape:", spike_counts.shape)
        #         print("First few spike counts:")
        #         print(spike_counts[0, :8, :8])  # Batch 0, channel 0, 10x10 pixel đầu
        input = input.float()
