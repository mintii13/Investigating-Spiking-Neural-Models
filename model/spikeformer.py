...
class SpikeDrivenTransformer(nn.Module):
    def __init__(
        ...
        pretrained=False,
        pretrained_cfg=None,
        **kwargs  # Accept any additional kwargs for config logging
    ):
        super().__init__()
        ...
        if spike_mode in ["lif", "alif", "blif"]:
            ...
            self.head_lif = MultiStepIFNode(v_threshold=0.8, v_reset=v_reset, detach_reset=True, backend="cupy")
    ...
    def _log_config(self, config_dict):
        """Log all configuration parameters"""
        logger = logging.getLogger()
        
        # Remove unwanted keys
        exclude_keys = {'self', '__class__', 'kwargs', 'norm_layer'}
        config_to_log = {k: v for k, v in config_dict.items() if k not in exclude_keys}
        ...
        for key in sorted(config_to_log.keys()):
            value = config_to_log[key]
